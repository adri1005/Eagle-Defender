...
def signup():
    def signup_database():
        ...
        cur = conn.cursor()
        cur.execute(
            "CREATE TABLE IF NOT EXISTS test(id INTEGER PRIMARY KEY, name text, email text, password text, birth text)")
        # Verificar si el correo o el nombre de usuario ya existen
        cur.execute("SELECT * FROM test WHERE email=? OR name=?", (e2.get(), e1.get()))
        if cur.fetchone():
            # Usuario ya registrado
            l4 = Label(signup_window, text="Correo o nombre de usuario ya registrado", font="times 15")
            l4.grid(row=6, column=2)
        else:
            # Usuario nuevo, proceder con el registro
    ...
    signup_window = Tk()
    signup_window.geometry("600x250")
